...
def main():
    ...
    if command == "--log":
        ...
        print("Options :")
        print("  grey        : Applique un filtre noir et blanc sur l'image (pas de paramètre)")
        print("  rotate: <deg> : Applique une rotation de <deg> degrés")
        print("  dilate : <lvl> : Applique un effet de dilatation avec <lvl>")
...
def processing(input_folder, output_folder, filter):
    ...
    for filtre in tableau_filtre:
        if ":" in filtre:
            ...
            if len(tableau_filtre_et_param) > 3 and tableau_filtre_et_param[0] == "text":
                ...
                tableau_img = apply_filter_images(
                    tableau_img,
                    tableau_filtre_et_param[0],
                    [
                        tableau_filtre_et_param[1],
                        tableau_filtre_et_param[2],
                        tableau_filtre_et_param[3],
                    ],
                )
            else:
    ...
    for image in tableau_img:
        image[1].save(os.path.join(output_folder, image[0]))
